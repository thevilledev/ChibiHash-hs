module ChibiHash (chibihash64) where
...
load64le bytes = foldr (\(shift, b) acc -> acc .|. (fromIntegral b `shiftL` shift))
                      0
...
chibihash64 input seed = finalMix x
  where
    ...
    -- Initial state
    h0@[ha, hb, hc, hd] = [p1, p2, p3, seed]
    h0' = debug "Initial state" h0 h0

    -- Process full 32-byte blocks
    (h1, remaining) = processBlocks bytes h0'
    h1' = debug "After blocks" h1 h1

    -- Process remaining bytes
    h2 = processRemaining remaining len h1'
    h2' = debug "After remaining" h2 h2
...
processBlocks input h
    ...
    where
        ...
        processBlock block h = 
            foldl processLane h (zip [0..3] (chunksOf 8 block))
        ...
        processLane h (i, lane) =
            let v = load64le lane
                ...
                h' = take i h ++ [hi'] ++ drop (i + 1) h    -- Update current lane
...
processRemaining bytes len [a,b,c,d] =
    let -- First add length mix to h[0]
        ...
        h1@[_, hb', hc', hd'] = process8ByteChunks bytes' 1 [ha4, b, c, d]
...
-- | Debug helper to print intermediate hash states
-- Uses unsafePerformIO to print values during computation
debug :: String -> [Word64] -> a -> a
debug msg vals x = unsafePerformIO $ do
    putStrLn $ msg ++ ": " ++ show vals
    return x